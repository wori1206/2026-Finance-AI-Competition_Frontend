...
/**
 * 화면의 일정 종류 → 서버 `구분`(결제전/결제후/집행).
 *
 * 🔴 「증빙」은 결제 «뒤» 에 하는 일입니다. 나머지는 결제 전 준비입니다.
 *    잘못 넣으면 상세의 「집행 준비 / 결제 후 진행」 두 묶음이 서로 바뀝니다.
 *
 * 🔴 「집행」만 세 번째 값으로 나갑니다. 서버가 계획을 만들 때 집행예정일로 찍는
 *    줄이 `구분='집행'` 이고, `adapt.ts` 가 그걸 보고 캘린더 배지를 갈라 찍습니다
 *    (`t.구분 === "집행" ? "집행" : …`). 여기서 「결제후」로 접어 보내면 사용자가
 *    직접 만든 집행 일정만 새로고침 뒤에 배지가 바뀝니다 — 같은 종류가 만든 방식에
 *    따라 다르게 보이는 것이라 그 자체가 결함입니다.
 */
function 구분추정(type: ScheduleItem["type"]): "결제전" | "결제후" | "집행" {
  if (type === "집행") return "집행";
  return type === "증빙" ? "결제후" : "결제전";
}
